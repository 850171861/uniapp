<template>
	<view>
		<video v-show="showVideo" class="myVideo" id="myVideo" @fullscreenchange="viideoControl" :src="videoSrc" ></video>
	<view class="content">
		<!-- 好中差评 -->
		<view class="label">
			<view @tap="loadRatings(index,label)" :class="{'on':index == labelIndex}"
			 v-for="(label,index) in labelList" :key="index">
	...
	export default{
		...
		methods:{
			loadRatings(index,label){
					...
					var a = this.ratingsList.find(item => item.grade == label.type)
					if(a == undefined){
						return
					}
					this.ratingsList = []
